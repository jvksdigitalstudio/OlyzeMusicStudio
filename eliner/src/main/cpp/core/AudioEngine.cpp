...
bool AudioEngine::start(int performanceProfile) {
    ...
    return startLocked(performanceProfile);
}

bool AudioEngine::startLocked(int performanceProfile) {
    mLifecycleState.store(LifecycleState::Starting, std::memory_order_release);
    ...
    if (result != oboe::Result::OK) {
        ...
        if (result != oboe::Result::OK) {
            ...
            mLifecycleState.store(LifecycleState::Stopped, std::memory_order_release); // nunca se abrió nada — no hay nada que stopLocked() necesite limpiar
            return false;
        }
    }
    ...
    if (result != oboe::Result::OK) {
        LOGE("Failed to start stream: %s — cleaning up before returning", oboe::convertToText(result));
        // ── Fase 1 de estabilización — Objetivo F ──────────────────────
        // Antes de este fix, un fallo aquí dejaba `mStream` ABIERTO (nunca
        // se llamaba close()/reset()) y el DSP graph que buildDspGraph()
        // ya había construido (líneas arriba, mVoices/mMixer/mFxChain)
        // sin liberar — un recurso de sistema huérfano (el stream Oboe/
        // AAudio) más una fuga de los objetos DSP en cada intento fallido
        // de start(). Reutilizar stopLocked() aquí (no el stop() público
        // — ya estamos bajo mLifecycleMutex, ver Objetivo D/E), en vez de
        // duplicar su lógica, garantiza el mismo cierre completo y ya
        // verificado que se usa en el path normal de parada:
        // requestStop()+close()+reset() del stream, collectGarbage()+
        // clear() del FX chain, y todos los flags de estado vueltos a
        // consistentes — dejando al motor listo para un reintento limpio
        // de start() sin recursos huérfanos.
        mLifecycleState.store(LifecycleState::Failed, std::memory_order_release);
        stopLocked(); // stopLocked() deja mLifecycleState en Stopped al final — ver su propio cuerpo.
        return false;
    }

    mIsRunning.store(true);
    mLifecycleState.store(LifecycleState::Running, std::memory_order_release);
...
void AudioEngine::stopLocked() {
    // Solo marca "Stopping" si veníamos de un estado realmente activo —
    // si stopLocked() se llama desde el path de fallo de startLocked()
    // (ya en Failed) o desde un segundo stop() redundante (ya en Stopped,
    // p. ej. el destructor tras un stop() explícito previo — ver ~AudioEngine()),
    // no tiene sentido anunciar una transición intermedia que no ocurrió.
    auto current = mLifecycleState.load(std::memory_order_acquire);
    if (current == LifecycleState::Running || current == LifecycleState::Recovering) {
        mLifecycleState.store(LifecycleState::Stopping, std::memory_order_release);
    }
    ...
    // ── Fase 1 — cierre: semántica de FAILED (sección 8 del prompt de
    // cierre) ────────────────────────────────────────────────────────
    // Decisión explícita: FAILED es un estado OBSERVABLE que persiste
    // hasta el siguiente start() explícito — NO un estado puramente
    // transitorio que stopLocked() sobrescribe de inmediato. Antes de
    // este fix, cualquier fallo de arranque (ver startLocked(): fallo de
    // requestStart() -> store(Failed) -> stopLocked()) quedaba
    // INVISIBLE para cualquier observador externo de getLifecycleState():
    // la única escritura de Failed ocurría microsegundos antes de que
    // esta misma función, incondicionalmente, la sobrescribiera con
    // Stopped — dentro del mismo lock_guard, sin ningún punto donde otro
    // hilo pudiera observar Failed. Un futuro diagnóstico en Kotlin
    // ("¿por qué no está sonando?") habría visto siempre Stopped,
    // indistinguible de un stop() intencional del usuario.
    //
    // Semántica elegida: si el estado de ENTRADA a esta función (`current`,
    // ya leído arriba) era Failed, stopLocked() NO lo reemplaza — el
    // motor queda observable como Failed hasta que start() se llame de
    // nuevo (transición Failed -> Starting, ya documentada en el enum).
    // Para cualquier otro estado de entrada (Running/Recovering/Stopping/
    // ya Stopped), el resultado sigue siendo Stopped, como antes.
    if (current != LifecycleState::Failed) {
        mLifecycleState.store(LifecycleState::Stopped, std::memory_order_release);
    }
...
bool AudioEngine::reopenStream() {
    // Objetivo D/E: un único lock_guard cubre TODO el ciclo stop+start de
    // la recuperación — así ningún otro hilo (JNI) puede intercalar su
    // propio start()/stop() a mitad de una recuperación en curso, y
    // viceversa. stopLocked()/startLocked() (no stop()/start() públicos)
    // porque el lock ya está tomado aquí — re-tomarlo sería un deadlock.
    std::lock_guard<std::mutex> lock(mLifecycleMutex);
    // Objetivo C: marca el estado ANTES de empezar el ciclo interno de
    // stop+start — stopLocked() reconoce "Recovering" como un estado
    // activo (igual que "Running") y lo hace pasar por "Stopping"
    // primero, no un salto silencioso a "Stopped". El resultado final
    // (Running si startLocked() tiene éxito, o Failed->Stopped si no) es
    // el mismo ciclo normal de cualquier stop()+start() — "Recovering" es
    // el marcador de que ESTE ciclo en particular lo disparó Oboe, no el
    // hilo de control, útil para diagnóstico/logging futuro.
    mLifecycleState.store(LifecycleState::Recovering, std::memory_order_release);
    stopLocked();
    return startLocked(mPerformanceProfile);
}
