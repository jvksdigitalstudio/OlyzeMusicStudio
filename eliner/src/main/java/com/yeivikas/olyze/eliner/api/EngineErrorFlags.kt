...
value class EngineErrorFlags(val bits: Int) {
    ...
    /** Mirrors native `kErrorRetireQueueFull` (AudioEngine.h) — a
     *  removed/replaced DSP module couldn't be pushed onto the native
     *  retire queue (it was full), so its memory leaked instead of being
     *  freed by `collectGarbage()`. This bit existed on the native side
     *  since Fase 7 but had no Kotlin-side accessor until this audit —
     *  every other native error bit already had one; a caller polling
     *  [lastError] for `bits and (1 shl 4)` by hand instead of a named
     *  property was the only way to observe it. */
    val retireQueueFull: Boolean get() = bits and (1 shl 4) != 0
