...
## Nota sobre MIDI de salida — MIGRADO a `:eliner` (ver ADR 0013)

`com.yeivikas.olyze.midi.OlyzeMidiManager` (en `:app`) ya no existe.
Reemplazado por `eliner.bridge.MidiOutputBridge`, que sí vive en
`:eliner` — no necesita `android.content.Context` en absoluto, porque no
importa `android.media.midi` directamente (eso sigue confinado a
`AndroidMidiBackend`, §26); solo depende de `EliNerMidiApi` (ya
construida por quien lo inyecta) y `TimeProvider`. El motivo que
mantenía esto fuera de `:eliner` — la dependencia directa de `Context` —
desapareció junto con la reimplementación duplicada.
...
(`com.yeivikas.olyze.midi.OlyzeMidiManager`, app → hardware externo,
migrado desde entonces a `eliner.bridge.MidiOutputBridge` — ver ADR 0013)
pero **cero** infraestructura de entrada — nada recibía eventos DE un
...
conectar ahí sonaría "aplicado" sin serlo, §20/§21). La migración de
`OlyzeMidiManager` a `EliNerMidiApi`, que esta fase dejó explícitamente
pendiente, se completó después — ver ADR 0013. Detalle completo, incluyendo qué quedó
...
## Estabilización del núcleo (concurrencia/lifecycle) — transversal, post MIDI Foundation

A diferencia de las fases anteriores (cada una añade una capa nueva), esta
fase no agrega funcionalidad: audita y corrige concurrencia y lifecycle
del núcleo ya construido. Resumen — detalle completo, con evidencia de
ejecución real (g++/ThreadSanitizer) y estado honesto por objetivo
(IMPLEMENTADO Y VERIFICADO / IMPLEMENTADO, NO EJECUTABLE EN ESTE ENTORNO /
DOCUMENTADO, NO IMPLEMENTADO), en `docs/adr/0014-fase1-estabilizacion-nucleo.md`:

- **`SpscCommandQueue` con 2 productores reales concurrentes** (UI +
  `eliner-dsp`/MidiRouter) sin serializar — violaba su propio contrato
  documentado. Confirmado con una prueba determinista de intercalación
  (no solo análisis), compilada y ejecutada realmente con g++. Corregido
  en Kotlin — `eliner.bridge.AudioCommandDispatcher` — no en el ring
  buffer C++, que se mantuvo intacto (sigue siendo genuinamente SPSC
  ahora que solo un hilo físico real llega a `push()`).
- **`gEngine` (JNI) sin sincronización** — mutex agregado en
  `EliNerAudioBridge.cpp`, patrón validado con ThreadSanitizer antes de
  aplicarlo.
- **El hilo interno de error de Oboe podía mutar `AudioEngine`
  concurrentemente con el hilo de control** (hallazgo adicional,
  descubierto durante la implementación, no en la auditoría inicial) —
  `mLifecycleMutex` interno, con el patrón `startLocked()`/`stopLocked()`
  para evitar deadlock (`start()` ya llama a `stop()` en su propio path
  de fallo).
- **`requestStart()` fallido dejaba el stream y el DSP graph huérfanos**
  — corregido reutilizando `stopLocked()`.
- **`MainViewModel` construía directamente toda la infraestructura de
  audio/MIDI** (threading, event bus, bridges) — extraída a
  `com.yeivikas.olyze.AppServices` (Application Services); `MainViewModel`
  queda con solo UI state + comandos.
- **Lifecycle FSM formal** (`LifecycleState`) añadido de forma aditiva en
  `AudioEngine` — no reemplaza los flags existentes.

**Qué NO se hizo en esta fase:** Parameter Architecture (Objetivo J) —
solo estrategia documentada en el ADR, cero código; retire queue
(`mRetireQueue`) no revisitado; ningún cambio se compiló contra el
toolchain real (NDK+Gradle, no disponible en el entorno de esta fase) —
verificación manual rigurosa más ejecución real de todo lo que no
depende de Android/NDK (`CommandQueue.h` es C++ puro).

**Cierre y verificación final:** una auditoría posterior encontró un bug
real en `AudioCommandDispatcher` (rechazo tras `close()` podía propagar
`RejectedExecutionException` hacia el llamador) y pidió verificación
ejecutable adicional de varios puntos que antes solo estaban
documentados por inspección — incluyendo el Retire Queue (antes no
auditado a fondo, ahora con evidencia real de AddressSanitizer/
ThreadSanitizer) y la semántica de `FAILED` en el lifecycle FSM (antes
transitorio e invisible para diagnóstico, ahora persistente por
decisión explícita). Detalle completo en
`docs/adr/0015-fase1-cierre-verificacion.md`.
