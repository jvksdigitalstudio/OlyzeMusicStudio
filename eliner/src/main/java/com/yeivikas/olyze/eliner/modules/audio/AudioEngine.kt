...
/**
 * ⚠️ NOT the audio path this app actually uses. Kept for historical/
 * architectural continuity from Fases 1-3, when this Kotlin module was
 * meant to become the real audio engine. Since Fase 4-6, the real,
 * production audio path is the native C++ engine
 * (`eliner/include/eliner/core/AudioEngine.h`, driven by Oboe, reached via
 * `eliner.bridge.EliNerAudioBridge`/JNI) — see
 * `docs/adr/0010-fase6-frontera-native-dsp.md` for why, and Fase 7
 * (`docs/adr/0011-fase7-dsp-graph-real.md`) for what was built on top of
 * it since. This class has zero callers anywhere in `:app` (confirmed via
 * a hardening-pass audit) and is not instantiated in production. It's
 * left in place, unmodified beyond this notice, because deleting working
 * (if unused) infrastructure isn't this hardening pass's job — that's a
 * call for whoever eventually decides this stack's fate (ADR 0010,
 * opción C). The original Fase 3-era description below is preserved as
 * written, for that same reason — it was accurate for what this class
 * was meant to become, just not for what actually shipped.
 *
 * ──────────────────────────────────────────────────────────────────────
 *
