 ...
 *
 * [context] is typed as [Application], not [Context] — same hardening
 * rationale as `DeviceCapabilityManager` (Fase — hardening pass): this
 * class retains it for its own lifetime, so the type itself should rule
 * out an Activity being passed in, not just a doc comment.
 */
class AudioDeviceManager(private val context: Application) : AudioDeviceProvider {
