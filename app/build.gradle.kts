...
android {
    ...
    // Pinned to exactly what .github/workflows/build.yml installs and
    // points ANDROID_NDK_HOME at (hardening pass: previously this was
    // only pinned in the CI script's `sdkmanager --install` step, not
    // here — so a local build, or a future CI change that forgets the
    // env var, could silently pick a different NDK than the one this
    // project is actually verified to build with).
