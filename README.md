...
│       │   ├── MainActivity.kt
│       │   ├── AppServices.kt            → Application Services — construye/posee audio+MIDI infra (ADR 0014, Objetivo H)
│       │   ├── MainViewModel.kt          → solo UI state + comandos — ya no construye infraestructura (ADR 0014)
...
│       │   ├── bridge/                  → EliNerAudioBridge.kt (implementación JNI real, gEngine ahora protegido por mutex — ver ADR 0014),
│       │   │                              AudioCommandDispatcher.kt (serializa comandos a un único hilo; contrato de cierre corregido y verificado — ADR 0014/0015),
│       │   │                              MidiOutputBridge.kt (MIDI salida), MidiToSynthBridge.kt (MIDI entrada → sintetizador)
